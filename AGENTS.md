...
- `viajante dates ORIGIN-DEST --from DATE --to DATE` is the cheapest-per-day table (date-grid RPC, 31-day cap) with an English compact week calendar and sparkline of owned daily prices. `--nights N` / `--trip rt` prices one packaged stay per outbound day. JSON `summary` (min/median/max, cheapest date, n_priced) is omitted when fewer than three days are priced. A compact calendar miss falls back to one shopping sweep per day on the same TLS session. `viajante explore ORIGIN --from DATE --days N` is the dest shortlist from the Explore catalog, then a priced `--top`. `viajante airports QUERY` is offline IATA lookup. `FlightQuery` rejects unknown codes.
