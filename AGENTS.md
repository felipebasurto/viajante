...
- The flight scrape locale is `hl=en` with `locale="en-US"` for stable rendered evidence and the existing JSON `locale: "en"` contract. Currency comes from `curr` (CLI/MCP `--currency`, default EUR), independently of `hl`. Country is optional `gl` (CLI/MCP `--country`); omit it when unset. Do not default `gl` to a home hub.
- `max_stops` is 0, 1, or 2 per query; filtering follows each query's value. `--max-layover` / `--min-layover` (hours) filter connecting offers by layover on both sweep and detail; nonstops stay. `--via` / `--exclude-via` keep or drop connecting offers whose parsed layover matches those IATA codes (city name or code); there is no owned shopping-POST via slot. Unknown layover cannot prove `--via` (drop) and cannot prove `--exclude-via` (keep). `--max-duration` drops offers whose elapsed time exceeds that many hours. Default trip kind is one-way. `ORIGIN-DEST:OUT:BACK` without `--trip` is sugar for two one-way queries (out then return). `--trip rt` / `round-trip` and `--trip multi` POST one packaged shopping request. `--nearby` expands origin or dest to owned same-city IATA and searches each as a labeled alternative (default off; named open-jaw airports stay). `adults`, `children`, `infants_in_seat`, `infants_on_lap`, and `cabin` are query fields (CLI `--adults` / `--children` / `--infants-in-seat` / `--infants-on-lap` / `--cabin`). Shopping constraints index 6 is `[adults, children, infants_in_seat, infants_on_lap]`. `--bags N` / `--carry-on` fill shopping constraints index 10 with the bags pair `[checked, carry_on]`; leave both unset (today's default) so that slot stays `None`. Index 7 is the price-cap hole and stays `None` unless we later add a cap.
