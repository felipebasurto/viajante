...
| **77** | `viajante trip` / `search_trip` same owned flight post-filters as `search_flights` | Gate ok. Unittest 803. `viajante bench --prompts` fail:0 (193 prompts, 135 pass, 58 skip, 0 scored). `judge: skip`. `judge_mean:` blank — not invented. Cold `--help` this host main 311.4/305.1 → branch 306.2/305.1 (later 309.0/315.6; did not veto). `score_ms` 1666 is not the keep. |
