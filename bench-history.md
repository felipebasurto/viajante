...
| **87** | leftover shop-parity `google_flights_url` on dates/flex/explore: reuse owned helper; flex winning-day query + offers (`booking_token` wins); dates report + per-day query URL (compact date+route allowed); explore dests that already ran a shopping POST; catalog/origin-only omit if encode cannot run; never invent a token | Gate ok. `viajante bench --prompts` fail:0 (193 prompts, 135 pass, 58 skip, 0 scored). `judge: skip`. `judge_mean:` blank — not invented. Operator judges on the box — do not invent a mean. Cold `--help` this host main 336/310 → branch 325/315 (later 334/312; did not veto). `score_ms` 1846 is not the keep. |
| **88** | named `--arrive-before` / `--depart-after` (HH:MM) local post-filters on owned `FlightOffer.arrival` / `.departure`; unnamed stays unset; “morning” does not invent a clock; unknown clock cannot prove the bound (drop); compact calendar / Explore catalog stay unfiltered; planner copies named HH:MM onto flights/dates/flex/explore/trip | Gate ok. Unittest 942. `viajante bench --prompts` fail:0 (193 prompts, 135 pass, 58 skip, 0 scored). `judge: skip`. `judge_mean:` blank — not invented. Operator judges on the box — do not invent a mean. Cold `--help` this host origin/main 392.2/300.7 → branch 378.7/302.8 (later 304.6/299.4 → 307.1/298.4; did not veto). `score_ms` 1780 is not the keep. |
| **89** | named `--exclude-airports` (owned IATA list, same parse as via): explore drops catalog dests with those codes before shop (no invented replacement dest or fare); nearby cannot sneak an excluded same-city code back; dates/flex/flights named origin/dest in the list is empty (no invented substitute unless `--nearby` already owned a non-excluded same-city code); unnamed stays unset; planner copies named `exclude_airports` onto explore/dates/flex/flights/trip | Gate ok. Unittest 956. `viajante bench --prompts` fail:0 (193 prompts, 135 pass, 58 skip, 0 scored). `judge: skip`. `judge_mean:` blank — not invented. Operator judges on the box — do not invent a mean. Cold `--help` this host main 359.0/314.5 → branch 351.8/307.8 (did not veto). `score_ms` 1881 is not the keep. |
...
- **#88** named `--arrive-before` / `--depart-after` (`cursor/arrive-before-depart-after-7774`): local post-filters on owned `FlightOffer.arrival` / `.departure` after parse, before `--top`. Unnamed stays unset. “morning” / “late” does not invent a clock. Unknown clock cannot prove the bound (drop). Compact calendar / Explore catalog stay unfiltered. Planner copies named HH:MM onto flights/dates/flex/explore/trip. Does not redo #75–#87. Gate ok. Unittest 942. `viajante bench --prompts` fail:0 (193 prompts, 135 pass, 58 skip, 0 scored). `judge: skip` (no key). `judge_mean:` blank — not invented. Cold `--help` this host origin/main 392.2/300.7 → branch 378.7/302.8 (later 304.6/299.4 → 307.1/298.4; did not veto). `score_ms` 1780 is not the keep. Not a keep vs 97.9.
