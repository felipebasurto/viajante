...
uv run viajante explore ORIGIN --from YYYY-MM-DD [--days N] [--month YYYY-MM] [--top N] [--adults N] [--children N] [--infants-in-seat N] [--infants-on-lap N] [--cabin CABIN] [--currency CODE] [--country CC] [--max-stops {0,1}] [--bags N] [--carry-on] [--price-cap EUR] [--airlines CODES] [--exclude-airlines CODES] [--alliance NAMES] [--exclude-alliance NAMES] [--via CODES] [--exclude-via CODES] [--exclude-airports CODES] [--include-airports CODES] [--depart-window START-END] [--arrive-before HH:MM] [--depart-after HH:MM] [--max-layover HOURS] [--min-layover HOURS] [--max-duration HOURS] [--sort {ranked,fare,price,duration,departure,arrival}] [--nearby] [--save FILE]
...
- Never invent a fare, typical, token, or via list. Occupancy, alliance, depart-window, arrive-before, depart-after, via/exclude-via, exclude-airports, and include-airports are only what the prompt named (omit occupancy when there is no count; do not invent an alliance code, a clock, a via list, or an IATA from a city vibe / “Europe”). Dest-list IATA and dest-like prefer_airports copy onto explore include only; origin / via / overnight must not become dest includes. Overnight IST `keep_connect` is still `require_overnight ∪ via`. Around/±N is `viajante flex`; cheapest week is `viajante dates`; packaged RT stays stay packaged. Named `--sort duration` / fastest / earliest copy onto explore; unnamed and vibe words like “Europe” do not invent a sort or a clock.
...
- `viajante trip` / `search_trip` is flights then hotels, one lock. Print owned fare + stay + sum when dates overlap and both sides return prices. Omit `trip_total` if either side misses, dates do not overlap, or currencies differ. Flight shop uses the same owned bags/via/exclude-airports/include-airports/airlines/`price-cap` post-filters as `search_flights`; unnamed stays unset. Never invent a missing side.
