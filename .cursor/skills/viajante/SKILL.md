...
- Never invent a fare, typical, token, or via list. Occupancy, alliance, depart-window, arrive-before, depart-after, via/exclude-via, no-overnight/require-overnight, exclude-airports, and include-airports are only what the prompt named (omit occupancy when there is no count; do not invent an alliance code, a clock, a via list, an overnight city, or an IATA from a city vibe / “Europe”). Dest-list IATA and dest-like prefer_airports copy onto explore include only; origin / via / overnight must not become dest includes. Overnight IST `keep_connect` is still `require_overnight ∪ via`. Named overnight copies onto dates/flex/explore; unnamed stays unset. Around/±N is `viajante flex`; cheapest week is `viajante dates`; packaged RT stays stay packaged. Named `--sort duration` / fastest / earliest copy onto explore; unnamed and vibe words like “Europe” do not invent a sort or a clock.
