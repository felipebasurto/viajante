...
`viajante bench` is the weekday speed loop (`gate` + `score_ms`). `--prompts` is the graded quality contract (offline deterministic tiers; DeepSeek 1–100 judge is opt-in via `VIAJANTE_BENCH_JUDGE=1` and `DEEPSEEK_API_KEY` / `VIAJANTE_JUDGE_KEY`). Rows print `plan_ms`; optional `VIAJANTE_BENCH_SWEEP=1` / `--timeit-sweep` times HTTP sweep for up to 8 planned flights (never the keep metric). Do not delete `tests/prompts/` to make the speed loop look better.
