...
- Never invent a fare, typical, token, or via list. Occupancy, alliance, depart-window, arrive-before, depart-after, and via/exclude-via are only what the prompt named (omit occupancy when there is no count; do not invent an alliance code, a clock, or a via list). Overnight IST `keep_connect` is still `require_overnight ∪ via`. Around/±N is `viajante flex`; cheapest week is `viajante dates`; packaged RT stays stay packaged.
