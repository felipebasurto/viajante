...
- Never invent a fare, typical, token, or via list. Occupancy, alliance, depart-window, arrive-before, depart-after, via/exclude-via, and exclude-airports are only what the prompt named (omit occupancy when there is no count; do not invent an alliance code, a clock, a via list, or an IATA from a city vibe). Overnight IST `keep_connect` is still `require_overnight ∪ via`. Around/±N is `viajante flex`; cheapest week is `viajante dates`; packaged RT stays stay packaged.
