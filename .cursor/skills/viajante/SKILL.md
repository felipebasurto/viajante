...
Local flight and hotel search from any IATA pair or city. No implied home hub. Currency is `--currency` / MCP `currency`, or inferred from a named origin’s owned country (JFK USD, LHR GBP, NRT JPY, GRU BRL). If unknown, ask. Hotels require `--currency` (no origin airport). If country, destination, or currency is not proven (a city with several airports, “Europe”, unnamed origin, two possible currencies), ask or error. Unknown cannot prove include. Do not invent IATA, `gl`, or ISO 4217 from vibe. Viajante does not convert; the MCP caller does FX. Flights default to one-way, one adult, economy; `--trip rt` packages a round-trip. Hotel prices are totals for the full stay. CLI hotels default to Booking; MCP hotels default to Google.

Fetch locale is English (`hl=en` / `lang=en`, `locale=en-US`). User prompts may be any language; planned queries stay English IATA and English fetch. Product docs and replies stay English. A Spanish (or other) prompt is labeled planner *input*, not product voice. Never invent a fare, typical, token, or via list.
...
uv run viajante hotels Tokyo 2026-12-04 2026-12-07 --currency JPY --top 3
```
...
Do **not** open with a full outbound×return date matrix across many cities. There is no implied home hub: use the origin the user named. If origin is unnamed, ask. If country, destination, or currency is not proven (a city with several airports, “Europe”, two possible currencies), ask or error. Do not pick. Unknown cannot prove include. Do not invent IATA, `gl`, ISO 4217, or a fare band from another origin. Prefer a sweep shortlist on fixed dates, then `--fetch detail` on 1–3 finalists. Prefer:

1. **Shortlist from vibe + a rough band for that named origin only.** If origin is unnamed, ask. Drop cities that are clearly out of budget before scraping. Do not reuse a band from a different origin. Do not invent a fare.
2. **Fixed natural dates first** — one outbound + one return per destination (e.g. Fri→Tue for a Monday holiday). `--top 3`, `--save` if comparing.
...
After a live scrape, report owned numbers in the quote currency. Do not invent prices from a heuristic band.
...
- Never invent a fare, typical, token, or via list. Copy only what the prompt **named** (occupancy, alliance, clocks, via, overnight, include/exclude airports, exclude-regions, currency). Unnamed stays unset. Vibe words do not invent a clock, dest, alliance code, buffer, bags, IATA, `gl`, or ISO 4217. A city with several airports, “Europe”, unnamed origin, or two possible currencies: ask or error; do not pick. Unknown cannot prove include. Named `max_stops` 0 plus a named positive `min_layover` keeps both and stamps one English note. Around/±N is `viajante flex`; cheapest week is `viajante dates`; packaged RT stays packaged.
- Currency is `--currency` / MCP `currency`, or inferred from a named origin’s owned country. If unknown, ask. Hotels require `--currency`. Viajante does not convert; the MCP caller does FX.
...
3. Only quote a price if the page shows the **same dates, occupancy, and a visible total**. Snippets, ads, and “from N in some currency” are not quotes.
